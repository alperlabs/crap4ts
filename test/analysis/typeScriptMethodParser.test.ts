...
describe("parseMethods", () => {
  ...
  it("counts getters and setters", () => {
    ...
    expect(names(parse(source))).toEqual(["get value", "set value"]);
  });
  ...
  it("folds anonymous inline callbacks into the enclosing method", () => {
    ...
    expect(methods[0].complexity).toBe(2); // callback && counts toward outer
  });
  ...
  it("treats named nested arrows as separate boundaries", () => {
    ...
    const methods = parse(source);
    expect(names(methods)).toEqual(["helper", "outer"]);
    expect(methods.find((m) => m.name === "outer")!.complexity).toBe(1);
    expect(methods.find((m) => m.name === "helper")!.complexity).toBe(2);
  });
});
